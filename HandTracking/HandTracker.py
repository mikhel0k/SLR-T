...
class Tracker:
    ...
    letters = {
        'А': 1, 'Б': 2, 'В': 3, 'Г': 4, 'Д': 5, 'Ж': 6, 'З': 7, 'Е': 8, 'Ё': 9,
    ...
    def __init__(self, camera):
        ...
        self.mp_hands = mp.solutions.hands  # Загрузка модуля MediaPipe для работы с руками
        self.hands = self.mp_hands.Hands()  # Создание объекта для детекции и трекинга рук
        self.mp_draw = mp.solutions.drawing_utils  # Загрузка утилит для рисования
        self.cap = cv2.VideoCapture(camera)  # Открытие видеопотока с камеры
